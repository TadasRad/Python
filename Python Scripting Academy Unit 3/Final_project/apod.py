...
def parse_command_line():
    ...
    parser.add_argument('-k','--api_key', action='store', metavar="API_KEY", help= 'NASA developer key')
...
def create_date(datelist, surprise):
    ...
    earliest_date = date(month=6, day=16, year=1995)
    print("received datelist= {}".format(datelist))
    if datelist:
        try:
            listed_date = date(month = int(datelist[0]), year =int(datelist[2]) , day = int(datelist[1]))
            if listed_date <= date.today() and listed_date >= earliest_date:
                return listed_date
            else:
                return None
        except:
    ...
    elif datelist== None and surprise ==True:
        max_days = date.today()-earliest_date
        print(max_days)
        random_date = earliest_date+ timedelta(days=randint(1,max_days.days))
        print(random_date)
        return random_date
        
    elif datelist== None and surprise ==False:
...
def query_url(d, api_key):
    ...
    return base+api_key+'&date='+formated_date
...
def save_image(d, image):
    ...
    """
    current_dir = "/Users/tadas.radzevicius/Code/BVPy/Python Scripting Academy Unit 3/Final_project/"
    image_name = d.strftime("%Y-%m-%d")+".jpg"
